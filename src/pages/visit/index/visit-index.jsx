...
import { list, deleted } from "@/proxies/visit";
...
export default function VisitIndex() {
  ...
  const onSearch = () => {
    setVisits(
      visits.filter((a) =>
        String(a.nombre).toLowerCase().match(String(visitName).toLowerCase())
      )
    );
  };
      ...
      <section className="hero has-background-primary-light mb-4">
        <nav className="level">
          <div className="level-left">
            <div className="level-item">
              <div className="hero-body">
                ...
                <p className="subtitle">Registro y creacion de visitas</p>
              </div>
            </div>
          </div>
          ...
          <div className="level-right">
            <div className="level-item">
              <div className="hero-body">
                <Link className="button" to="/visit/create">
                  Registrar
                </Link>
              </div>
            </div>
        ...
        {isLoading ? (
          ...
          <div className="table-container">
            <table className="table is-hoverable is-fullwidth">
              ...
              <tbody>
                {visits.map((visit) => (
                  <tr key={visit.visitaId}>
                    <td>{visit.nombre}</td>
                    <td>{visit.apellido}</td>
                    <td>{visit.direccion}</td>
                    <td>{visit.fechaHoraIngreso}</td>
                    <td>{visit.fechaHoraSalida}</td>
                    <td>{visit.tipoDocumento}</td>
                    <td>{visit.numeroDocumento}</td>
                    <td>{visit.tipoVehiculo}</td>
                    <td>
                      <div className="buttons">
                        <Link
                          className="button is-small is-warning"
                          to={`/visit/edit/${visit.visitaId}`}
                        >
                          <svg
                            width="24"
                            height="24"
                        ...
                        <button
                          className={
                            isLoadingDelete
                              ? "button is-small is-danger is-loading"
                              : "button is-small is-danger"
                          }
                          onClick={() => onDelete(visit.visitaId, visit)}
                        >
                          <svg
                            width="24"
                            height="24"
